...
contract MultiiSig {
    ...
    modifier tokenExists(string memory ticker) {
        if (keccak256(bytes(ticker)) != keccak256(bytes("ETH"))) {
            require(
                ...
                "token does not exixts"
            );
        }
        _;
    }

    function addToken(string memory ticker, address _tokenAddress)
        public
        onlyowners
    {
        for (uint256 i = 0; i < tokenList.length; i++) {
            require(
                keccak256(bytes(tokenList[i])) != keccak256(bytes(ticker)),
    ...
    function callAddOwner(address owner, address multiSigContractInstance)
        private
    {
    ...
    function callRemoveOwner(address owner, address multiSigContractInstance)
        private
    {
        MultiSigFactory factory = MultiSigFactory(multisigIntance);
    ...
    function deposit(string memory ticker, uint256 amount)
        public
        payable
        onlyowners
        tokenExists(ticker)
    {
        require(
            balance[msg.sender][ticker] >= 0,
            "cannot deposiit a calue of 0"
        );
    ...
    function withdraw(string memory ticker, uint256 amount)
        public
        onlyowners
        tokenExists(ticker)
    {
    ...
    function createTrnasferRequest(
        string memory ticker,
        address payable receiver,
        uint256 amount
    ) public onlyowners tokenExists(ticker) {
        require(
            balance[msg.sender][ticker] >= amount,
            "insufficent funds to create a transfer"
        );

        for (uint256 i = 0; i < walletowners.length; i++) {
            require(
                walletowners[i] != receiver,
                "cannot transfer funds withiwn the wallet"
            );
        }
    ...
    function cancelTransferRequest(uint256 id) public onlyowners {
        ...
        require(
            transferRequests[transferIndex].sender == msg.sender,
            "only the transfer creator can cancel"
        );
        require(hasBeenFound, "transfer request does not exist");

        balance[msg.sender][ticker] += transferRequests[transferIndex].amount;

        transferRequests[transferIndex] = transferRequests[
            transferRequests.length - 1
        ];
    ...
    function approveTransferRequest(uint256 id) public onlyowners {
        string memory ticker = transferRequests[id].ticker;
        bool hasBeenFound = false;
        uint256 transferIndex = 0;
        for (uint256 i = 0; i < transferRequests.length; i++) {
    ...
    function transferFunds(string memory ticker, uint256 id) private {
        balance[transferRequests[id].receiver][ticker] += transferRequests[id]
            .amount;
        ...
        emit fundsTransfered(
            ...
            transferRequests[id].timeOfTransaction
        );
    ...
    function getApprovals(uint256 id) public view returns (bool) {
    ...
    function getBalance(string memory ticker)
        public
        view
        tokenExists(ticker)
        returns (uint256)
    ...
    function getContractERC20Balance(string memory ticker)
        public
        view
        tokenExists(ticker)
        returns (uint256)
...
contract MultiSigFactory {
    ...
    event WalletCreated(
        address createdBy,
        address newWalletContractAddress,
        uint256 timeOfTransaction
    );
    ...
    function removeNewWalletInstance(address _owner, address _walletAddress)
        public
    {
        ...
        bool hasBeenFound = false;
        uint256 walletIndex;
        for (uint256 i = 0; i < newWallet.length; i++) {
